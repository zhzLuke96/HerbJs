...
import { isDOM, UniqueId, flatten, isDef, isUnDef } from '../common';
...
function createFragByText(text: string, vmap: ValueMap, container: HTMLElement | DocumentFragment): (() => void)[] {
    ...
    getArrByRegFromString(pointRe, text, text => {
        ...
        execValue(value, text)
    })
    return depFuncs

    function execValue(value, text = '') {
        switch (typeof value) {
            case 'undefined':
                appendTextChild(container, text)
                return
            case 'string':
                // 字符串节点
                    ...
                    depFuncs.push(() => {
                        ...
                        value().forEach(val => {
                            if (typeof val === 'string' || val instanceof String) {
                                const t = createTextNode(val)
                                if (!t) {
                                    return
                                }
                                funcfrag.append(t as unknown as HTMLElement)
                            } else if (val instanceof Function) {
                                const f = val()
                                if (f instanceof DocumentFragment) {
                                    funcfrag.append(f as unknown as HTMLElement)
                                    return
                                }
                                if (typeof f === 'string' || f instanceof String) {
                                    const t = createTextNode(f)
                                    if (!t) {
                                        return
                                    }
                                    funcfrag.append(t as unknown as HTMLElement)
                ...
                depFuncs.push(() => {
                    ...
                    if (typeof newResult !== 'function') {
                        if (!funcfrag.has(textNode as unknown as HTMLElement)) {
                            funcfrag.clear()
                            funcfrag.append(textNode as unknown as HTMLElement)
                        }
                    ...
                    funcfrag.clear()
                    let fragNode = (newResult as unknown as () => any)()
                    if (fragNode instanceof DocumentFragment) {
                        funcfrag.replace(fragNode)
                    } else {
                        fragNode = createTextNode(fragNode)
                        funcfrag.clear()
                        funcfrag.append(fragNode as unknown as HTMLElement)
                    }
...
function setElemAttr(elem: HTMLElement, key: string, val: any, vmap: ValueMap, depFuncs: (() => void)[]) {
...
function setElemRef(elem, refs: any) {
    ...
    if (Array.isArray(refs)) {
        flatten(refs).filter(x => typeof x === 'function').forEach(ref => ref(elem))
        return
    }
}
...
function attrSetter(elem: HTMLElement, key: string, val: any) {
    ...
    if (key === 'class' || key === 'className') {
        if (elem.classList) {
            ...
            newCls.filter(t => !!t.trim())
                .forEach(cls => elem.classList.add(cls))
            if (oldCls) {
                oldCls.filter(cls => !newCls.includes(cls))
                    .filter(t => !!t.trim())
