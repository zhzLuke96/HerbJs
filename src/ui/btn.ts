...
export const Button = (props: ButtonProps = {}) => {
    ...
    const { motionRef, start: activeStart } = useMotion(type === 'danger' ? dangerActiveMotion : activeMotion)
    ...
    return html`
        <button
            ${excludeKeysObj(props, ['type', 'text', 'style', 'shape', 'icon', 'ref', 'disabled', 'loading'])}
            type='button'
            ref=${[styleRef, propsStyleRef, motionRef, mouseupRef, ref]}
            disabled=${() => GetValue(disabled)}
            class=${() => GetValue(loading) ? 'loading' : ''}
        >
            ${() => !GetValue(loading) ? '' : Icon({ name: 'donut_large', style: { animation: 'loadingCircle 1s infinite linear' } })}
