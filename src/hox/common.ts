...
const _isDef = (o) => o !== undefined && o !== null
...
export const isDef = (arr: any, ...arg: any[]): boolean => {
    ...
    arr = [...arr, ...arg]
    return arr.length ? arr.reduce((r, o) => r && _isDef(o), true) : false;
}
...
const performanceHEX = () => shuffleStr(parseInt(performance.now().toString().replace('.', ''), 10).toString(16));
const DateHEX = () => shuffleStr(Date.now().toString(16));
const RandomHEX = () => shuffleStr((Math.random()).toString(16).slice(2));
export const UniqueId = (gap = '_', unit = 6, part = 3) => shuffle(Array.from(`${performanceHEX()}${DateHEX()}${RandomHEX()}`.matchAll(new RegExp(`.{${unit}}`, 'g')))).slice(0, part).join(gap)
